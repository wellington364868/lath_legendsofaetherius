Scriptname LATH_MGE_Flamebearer extends activemagiceffect  
...
Event OnEffectStart(Actor akTarget, Actor akCaster)
    ...
    ;se weather_factor for 0 o clima atual nao permtite bonus
    ...
    float fire_resist = akTarget.GetAV("ResistFire")
    float magic_resist = akTarget.GetAV("ResistMagic")

    float fire_resist_mult =  (1.0 - (fire_resist / 100.0)) 
    float magic_resist_mult = (1.0 - (magic_resist / 100.0))

    float final_damage = BaseDamage * skillMult * perkMult * weather_factor * fire_resist_mult * magic_resist_mult


    ;fator de aleatoriedade
    ;FOGO: RANK1-> 25%   RANK2-> 15%

    float random_final_damage = 0.0
    if PlayerRef.HasPerk(Weather_2)
        random_final_damage = Utility.RandomFloat(final_damage * 0.85, final_damage)
    elseif PlayerRef.HasPerk(Weather_1)
        random_final_damage = Utility.RandomFloat(final_damage * 0.75, final_damage)
    endif
...
Event OnEffectFinish(Actor akTarget, Actor akCaster)
    ...
    Float damageDealt = target_hp - last_value
        ...
        if damageDealt < -1.0 ;recebeu dano significativo
            Debug.Notification("Dano [FOGO]: " + Math.Floor(damageDealt))
        endif
EndEvent
