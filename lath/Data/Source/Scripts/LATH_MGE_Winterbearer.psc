Scriptname LATH_MGE_Winterbearer extends activemagiceffect  
...
Event OnEffectStart(Actor akTarget, Actor akCaster)
    ...
    if PlayerRef.IsInInterior()
        if PlayerRef.HasPerk(Weather_2) ;bonus 15% mais fraco
            weather_factor =  weather_factor * 0.85
        elseif PlayerRef.HasPerk(Weather_1); bonua 60% mais fraco
            weather_factor =  weather_factor * 0.60
        endif
    ...
    float frost_resist = akTarget.GetAV("ResistFrost")
    float magic_resist = akTarget.GetAV("ResistMagic")

    float frost_resist_mult =  (1.0 - (frost_resist / 100.0)) 
    ...
    float final_damage = BaseDamage * skillMult * perkMult * weather_factor * frost_resist_mult * magic_resist_mult

    ;fator de aleatoriedade - BAIXO
    ;FROST: RANK1-> 10%   RANK2-> 5%

    float random_final_damage = 0.0
    if PlayerRef.HasPerk(Weather_2)
        random_final_damage = Utility.RandomFloat(final_damage * 0.95, final_damage)
    elseif PlayerRef.HasPerk(Weather_1)
        random_final_damage = Utility.RandomFloat(final_damage * 0.90, final_damage)
    endif
...
Event OnEffectFinish(Actor akTarget, Actor akCaster)
    ...
    Float damageDealt = target_hp - last_value
        ...
        if damageDealt < -1.0 ;recebeu dano significativo
            Debug.Notification("Dano [GELO]: " + Math.Floor(damageDealt))
        endif
EndEvent
